...
assert.match(batch, /const BATCH_MAX_BUFFERED_FORMATS: u32 = 8/);
assert.match(batch, /await_batch_printer_ready\([\s\S]*?query_printer_status_with_sink/);
assert.match(batch, /"paper-jam"[\s\S]*?"paper-out"[\s\S]*?"paused"[\s\S]*?"buffer-full"/);
assert.match(batch, /fixed-batch-printer-wait/);
assert.match(batch, /fixed-batch-printer-ready/);
assert.match(batch, /status_wait_us/);
assert.match(transport, /\*breaker_until = None;[\s\S]*?status_report: Some/);
assert.match(slint, /fixed-batch-printer-wait[\s\S]*?ui\.set_fixed_status/);
...
assert.match(pack, /record_pack_with_outbox_checked\(\s*&counters/);
...
assert.match(operational, /transaction_with_behavior\(rusqlite::TransactionBehavior::Immediate\)[\s\S]*?counter_snapshot\(&transaction, Some\(payload\.nomenclature_id\)\)[\s\S]*?actual != \*expected_counters[\s\S]*?return Ok\(None\)/);
