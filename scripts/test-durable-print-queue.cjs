...
assert.match(durable, /MAX_RECOVERY_JOBS: usize = 5_000/);
assert.match(durable, /MAX_LIST_JOBS: usize = 5_000/);
assert.match(durable, /UPDATE pack SET status = 'Printed'/);
assert.match(durable, /soft-delete pack after delivery cancellation/);
assert.match(durable, /restore linked pack for delivery retry/);
assert.match(durable, /release terminal pack delivery links/);
assert.match(processor, /delivery_job_id TEXT REFERENCES printer_delivery_jobs\(job_id\)/);
assert.match(operational, /status = 'Pending'/);
...
assert.match(printer, /durable\.mark_uncertain\(job_id, &error\.message\)/);
assert.match(printer, /durable\.mark_retry_queued\(job_id, &error\.message\)/);
assert.match(printer, /TransportFailureKind::NotStarted/);
assert.match(printer, /error\.is_uncertain\(\)/);
assert.match(durable, /mark_terminal_error\(job_id, "failed", error, &\["queued", "sending"\]\)/);
...
console.log('Durable print queue: 7 explicit states, 5,000-job recovery/list bound, 30-day terminal retention');
