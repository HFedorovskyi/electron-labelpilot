...
const DeleteItemsModal: React.FC<DeleteItemsModalProps> = ({ isOpen, onClose, onDeleted }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl w-full max-w-4xl h-[80vh] flex flex-col shadow-sm dark:shadow-2xl relative overflow-hidden transition-colors">
                ...
                {/* Tabs */}
                <div className="flex border-b border-neutral-200 dark:border-neutral-800 bg-neutral-100/50 dark:bg-neutral-900/30">
                    <button
                ...
                {/* Content */}
                <div className="flex-1 overflow-auto p-4 bg-neutral-50/50 dark:bg-neutral-900/30">
                    {loading ? (
                                                    ...
                                                    >
                                                        <div className="flex flex-col">
                                                            <div className="flex items-center gap-3">
                                                                <span className="font-mono text-xl font-medium text-neutral-900 dark:text-white">{pack.number}</span>
                                                                {pack.status === 'Deleted' && (
                                                                    <span className="px-2 py-0.5 text-xs bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 rounded-full border border-red-200 dark:border-red-500/20">{t('delete.deleted')}</span>
                                                                )}
                                                            </div>
                                                            <div className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
                                                                {t('delete.weight')} <span className="text-neutral-900 dark:text-white">{pack.weight_netto.toFixed(3)} {t('ws.kg')}</span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        {pack.status !== 'Deleted' && (
                                                            <button
                                                                ...
                                                                className="p-3 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 text-red-600 dark:text-red-500 rounded-lg border border-red-200 dark:border-red-500/20 transition-colors"
                                                            >
                                                                <Trash2 size={24} />
                                                            </button>
                                                        )}
                                        ...
                                        <>
                                            ...
                                            <div className="space-y-2">
                                                {data.boxesInPallet?.map((box: any) => (
                                                    <div
                                                        key={box.id}
                                                        className={`flex items-center justify-between p-4 rounded-xl border transition-all shadow-sm dark:shadow-none ${box.status === 'Deleted'
                                                            ? 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-900/30 opacity-60'
                                                            : box.status === 'Open'
                                                    ...
                                                    >
                                                        <div className="flex flex-col">
                                                            <div className="flex items-center gap-3">
                                                                <span className="font-mono text-xl font-medium text-neutral-900 dark:text-white">{box.number}</span>
                                                                {box.status === 'Deleted' ? (
                                                                    <span className="px-2 py-0.5 text-xs bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 rounded-full border border-red-200 dark:border-red-500/20">{t('delete.deleted')}</span>
                                                                ) : box.status === 'Open' ? (
                                                                    <span className="px-2 py-0.5 text-xs bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-full border border-emerald-200 dark:border-emerald-500/20">{t('delete.open')}</span>
                                                                ) : (
                                                                    <span className="px-2 py-0.5 text-xs bg-neutral-200 dark:bg-neutral-700 text-neutral-600 dark:text-neutral-400 rounded-full">{t('delete.closed')}</span>
                                                                )}
                                                            </div>
                                                            <div className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
                                                                {t('delete.weight')} <span className="text-neutral-900 dark:text-white">{(box.weight_netto || 0).toFixed(3)} {t('ws.kg')}</span>
                                                            </div>
                                                        </div>
            ...
            {confirmModal.isOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-2xl w-full max-w-md p-8 shadow-sm dark:shadow-2xl flex flex-col items-center">
                        <div className="w-16 h-16 bg-red-50 dark:bg-red-500/10 rounded-full flex items-center justify-center mb-6 border border-red-200 dark:border-red-500/20">
                            <Trash2 size={32} className="text-red-600 dark:text-red-500" />
                        </div>
                        <h3 className="text-xl font-bold text-neutral-900 dark:text-white mb-2 text-center">{t('delete.title')}?</h3>
