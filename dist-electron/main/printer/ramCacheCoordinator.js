...
exports.ramCacheCoordinator = void 0;
...
class RamCacheCoordinator {
    ...
    getDecision(config) {
        ...
        if (!entry)
            return 'inline'; // first print → safe fallback
        if (Date.now() - entry.decidedAt > PROBE_TTL_MS)
            return 'inline'; // stale
    ...
    invalidate(printerId) {
        if (this.entries.delete(printerId)) {
            logger_1.default.info(`[RamCacheCoordinator] decision invalidated for "${printerId}"`);
        }
    }
    /**
     ...
     */
    maybeProbe(config, strategy) {
        const mode = config.ramCache || 'auto';
        if (mode !== 'auto')
            return; // explicit on/off doesn't need probing
        ...
        if (typeof strategy.query !== 'function') {
            ...
            logger_1.default.info(`[RamCacheCoordinator] "${config.id}" has no read channel → inline locked`);
            return;
        }
        this.entries.set(config.id, { decision: 'inline', state: 'probing', decidedAt: Date.now() });
        this.runProbe(config, strategy).catch((e) => {
            logger_1.default.warn(`[RamCacheCoordinator] probe crashed for "${config.id}": ${e}`);
            this.entries.set(config.id, { decision: 'inline', state: 'done', decidedAt: Date.now() });
        });
    }
    async runProbe(config, strategy) {
        if (!strategy.query)
            return;
        if (!strategy.isConnected()) {
            logger_1.default.info(`[RamCacheCoordinator] probe skipped for "${config.id}" — not connected`);
            return;
        }
        ...
        try {
            ...
            if (reply && reply.length > 0) {
                ...
                if (text.includes('LP_PROBE.GRF'))
                    decision = 'ram';
            }
            this.entries.set(config.id, { decision, state: 'done', decidedAt: Date.now() });
            logger_1.default.info(`[RamCacheCoordinator] probe done for "${config.id}" → ${decision} (reply ${reply?.length ?? 0}B)`);
            // Best-effort cleanup. Ignore errors — the file is 1 byte, no harm if it stays.
            try {
                if (strategy.isConnected())
                    await strategy.send(Buffer.from(cleanup, 'utf-8'));
            }
            catch { /* ignore */ }
        }
        catch (e) {
            logger_1.default.warn(`[RamCacheCoordinator] probe failed for "${config.id}": ${e}`);
            this.entries.set(config.id, { decision: 'inline', state: 'done', decidedAt: Date.now() });
        }
    }
}
exports.ramCacheCoordinator = new RamCacheCoordinator();
